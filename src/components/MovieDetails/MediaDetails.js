...
const MediaDetails = (props) => {
  ...
  const fetchData = async () => {
    ...
    const { data } = await axios.get(
      `https://api.themoviedb.org/3/${
        props.mediaType
      }/${id}?api_key=${"bf7b9eabb49e072d0c8f00d1a5542e14"}&language=en-US`
    );
    setMedia(data);
    setTimeout(() => {
      setLoading(false);
  ...
  const fetchRecomment = async () => {
    const { data } = await axios.get(
      `https://api.themoviedb.org/3/${
        props.mediaType
      }/${id}/recommendations?api_key=${"bf7b9eabb49e072d0c8f00d1a5542e14"}&language=en-US`
    );
    //console.log(data)
  ...
  const fetchReviews = async () => {
    const { data } = await axios.get(
      `https://api.themoviedb.org/3/${
        props.mediaType
      }/${id}/reviews?api_key=${"bf7b9eabb49e072d0c8f00d1a5542e14"}&language=en-US`
    );
    //console.log(data)
  ...
  return loading ? (
    ...
    media && (
      <div className="details_container">
        ...
        <div>
          <h2 className="detailsItem_title">
            {media.name ? media.name : media.original_title}
          </h2>
        </div>
            ...
            <div className="detailsItem_trailer">
              <div className="player">
                <ReactPlayer
                  ...
                  height={"500px"}
                  controls={true}
                  url={videoUrl !== undefined ? videoUrl : "asdasd"}
                />
              </div>{" "}
            </div>
          </div>
          ...
          <div className="detailsItem_credits">
            ...
            <div>
              ...
              <p className="language">
                Language:
                {media.spoken_languages.map((language, index) => (
                  <span key={index}>
                    {index < media.spoken_languages.length - 1
                      ? language.name + ","
                      : language.name}
                  </span>
                ))}
              ...
              <p className="country">
                ...
                {media.production_countries.map((country, index) => (
                  <span key={index}>
                    {index < media.production_countries.length - 1
                      ? country.name + ","
                      : country.name}
                  </span>
                ))}
              ...
              <div className="reviews_container">
                {reviews.map((review, index) => (
                  <ReviewCard
                    ...
                    authorInfo={review.author_details}
